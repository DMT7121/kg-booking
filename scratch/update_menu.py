...
file_path = "src/components/modals/MenuManagerModal.vue"
with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

# 1. Update activeTab and script functions
script_update = """const originalDishState = ref('')
...
content = re.sub(r"const activeTab = ref\('manage'\) // 'manage' \| 'add'", "", content)
content = re.sub(r"function close\(\) \{[\s\S]*?\}", "", content, count=1)
content = re.sub(r"function selectDish\(dish: any\) \{[\s\S]*?\}", script_update, content, count=1)

# Update saveSelectedDish and deleteSelectedDish to reset originalDishState
content = content.replace("selectedDish.value = null;", "selectedDish.value = null;\n  originalDishState.value = '';")

# Update handleDishImageUpload
upload_func = """async function handleDishImageUpload(event: Event) {
...
content = re.sub(r"function handleDishImageUpload\(event: Event\) \{[\s\S]*?reader\.readAsDataURL\(file\)\n\}", upload_func, content)


# 2. Update Template
# Remove the Tabs bar
content = re.sub(r"<!-- Tabs -->[\s\S]*?<!-- Scrollable Content Area -->", "<!-- Scrollable Content Area -->", content)

# Replace the separate tabs with a unified view
unified_view = """
          ...
          <div class="bg-white rounded-2xl border border-slate-200 p-4 mb-5 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center relative z-10 shrink-0">
            ...
            <div class="flex flex-wrap items-center gap-2 w-full md:w-auto">
              <button v-if="appStore.activeSheet" @click="prepareUpdate(appStore.activeSheet); showUploadModal = true" class="flex-1 md:flex-none px-4 py-2 bg-white border border-slate-200 text-slate-600 text-sm font-bold rounded-xl hover:bg-slate-50 transition-colors flex items-center justify-center gap-2 shadow-sm">
                <i class="fa-solid fa-pen text-blue-500"></i> <span class="hidden sm:inline">Sửa Menu</span>
              </button>
...
# Replace everything from `<!-- TAB 1: QUẢN LÝ THỰC ĐƠN -->` to right before `<!-- Right Side: Edit Panel -->`
pattern = r"<!-- TAB 1: QUẢN LÝ THỰC ĐƠN -->[\s\S]*?<!-- Right Side: Edit Panel -->"
content = re.sub(pattern, unified_view + "\n        <!-- Right Side: Edit Panel -->", content)

# Remove the closing `</div>` of TAB 2 since we removed its opening
# Wait, my `unified_view` opened `<div class="flex flex-col lg:flex-row h-full max-w-7xl mx-auto w-full relative overflow-hidden">`
# And `<!-- Right Side: Edit Panel -->` follows it, then it is closed by `</div>` which was originally closing `activeTab === 'add'`.
# So the tags should be balanced. Let's verify.

# Update "Thêm ảnh khác" to support taking photos
upload_box_new = """
            <!-- Upload Box -->
            <div class="flex gap-3 mb-6">
              <label class="flex-1 border-2 border-dashed border-blue-200 bg-blue-50/50 rounded-2xl p-4 text-center cursor-pointer hover:bg-blue-50 hover:border-blue-400 transition-all group">
                ...
                <h4 class="font-bold text-blue-700 text-xs">Tải ảnh lên</h4>
              </label>
              ...
              <label class="flex-1 border-2 border-dashed border-emerald-200 bg-emerald-50/50 rounded-2xl p-4 text-center cursor-pointer hover:bg-emerald-50 hover:border-emerald-400 transition-all group">
                ...
                <h4 class="font-bold text-emerald-700 text-xs">Chụp ảnh (1:1)</h4>
              </label>
            </div>
"""
content = re.sub(r"<!-- Upload Box -->[\s\S]*?<!-- Form Fields -->", upload_box_new + "\n            <!-- Form Fields -->", content)

# Update the "Đóng" buttons to use selectDish(null)
content = content.replace("@click=\"selectedDish = null\"", "@click=\"selectDish(null)\"")
