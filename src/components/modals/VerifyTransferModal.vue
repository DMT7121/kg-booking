...
<template>
  <div v-if="ui.verifyModal.show" class="fixed inset-0 bg-[#0D1658]/80 z-[12000] flex justify-center items-center p-4 backdrop-blur-md">
    <div class="bg-white rounded-3xl shadow-2xl p-6 md:p-8 max-w-sm w-[95%] md:w-full flex flex-col relative overflow-hidden border border-white/20">
      ...
      <div class="flex justify-center items-center mb-6 relative z-10 flex-col gap-3">
        ...
        <h3 class="text-xl font-black text-[#1A237E] uppercase tracking-tighter text-center">Số Tiền Không Khớp</h3>
      </div>
      ...
      <div class="space-y-4 mb-8 relative z-10">
        ...
        <div class="p-5 bg-blue-50/50 rounded-2xl border border-blue-100 shadow-sm relative overflow-hidden">
          <div class="absolute right-0 top-0 bottom-0 w-1 bg-[#1A237E]"></div>
          <div class="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">AI đọc được</div>
          <div class="text-2xl font-black text-[#1A237E]">{{ formatVND(ui.verifyModal.scanned.amount) }}</div>
      ...
      <div class="grid grid-cols-2 gap-3 relative z-10">
        ...
        <button @click="confirmVerification(true)" class="py-4 bg-[#1A237E] text-white rounded-xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-blue-900/20 hover:bg-blue-800 active:scale-95 transition-all flex items-center justify-center gap-1.5"><i class="fa-solid fa-check"></i> DÙNG SỐ AI</button>
      </div>
    </div>
  </div>
</template>
