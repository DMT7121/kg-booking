...
<template>
  <div class="bg-white rounded-3xl shadow-xl shadow-blue-900/5 border border-slate-100 p-5 md:p-6">
    <div class="flex justify-between items-center mb-6">
      <h3 class="font-black text-[#1A237E] text-xs uppercase tracking-widest flex items-center gap-3">
    ...
    <div class="space-y-4">
      <div v-for="(item, index) in formStore.items" :key="index" 
           ...
           :class="[
             ...
             dragOverIndex === index && draggedIndex !== index ? 'border-t-4 border-t-[#1A237E] pt-6 scale-[1.02] shadow-lg' : ''
           ]">
        <div class="flex flex-col gap-4">
          ...
          <div class="relative w-full flex items-center gap-3">
            <div class="flex flex-col gap-1 p-2 bg-slate-50 rounded-xl text-slate-400 border border-slate-100 -ml-2 opacity-50 group-hover:opacity-100 transition-opacity">
              <button @click="swapItem(index, index - 1)" :disabled="index === 0" class="hover:text-[#1A237E] disabled:opacity-20 active:scale-90 transition-transform"><i class="fa-solid fa-chevron-up text-[10px]"></i></button>
              <button @click="swapItem(index, index + 1)" :disabled="index === formStore.items.length - 1" class="hover:text-[#1A237E] disabled:opacity-20 active:scale-90 transition-transform"><i class="fa-solid fa-chevron-down text-[10px]"></i></button>
            </div>
            
            <div class="relative flex-grow">
              <input v-model="item.name" @input="onSearchInput(index)" @blur="handleItemBlur" @focus="handleInputFocus" class="w-full font-black text-[#1A237E] text-base md:text-sm border-b-2 border-slate-100 focus:border-[#1A237E] outline-none pb-2 uppercase placeholder-slate-300 transition-colors" placeholder="NHẬP TÊN MÓN...">
          ...
          <div class="flex gap-3 items-center justify-end">
            <div class="flex items-center gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-200 flex-grow md:flex-grow-0 justify-center shadow-inner">
              ...
              <input type="number" v-model="item.price" @focus="handleInputFocus" @blur="handleInputBlur" class="w-28 text-right font-black text-[#1A237E] bg-transparent text-base md:text-sm outline-none placeholder-slate-400" placeholder="Giá">
            </div>
    ...
    <div class="mt-6 border-t border-slate-100 pt-5 flex justify-between items-center px-2">
      ...
      <div class="text-sm font-black text-[#1A237E] bg-blue-50 px-3 py-1 rounded-lg" v-if="formStore.taxEnabled">{{ formatVND(formStore.calculatedTotals.tax) }}</div>
    </div>
  </div>
</template>
