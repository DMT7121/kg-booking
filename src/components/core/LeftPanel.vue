...
<template>
  ...
  >
    ...
    <!-- TABS -->
    <div class="flex flex-nowrap overflow-x-auto w-full bg-white text-[10px] font-black border-b border-slate-200 uppercase tracking-widest shadow-sm relative z-10 no-scrollbar">
      <button @click="ui.tab = 'timeline'; appStore.loadHistory(false)" :class="['flex-1 py-3 flex justify-center items-center gap-2 transition-all min-h-[48px] border-b-[3px] whitespace-nowrap px-2', ui.tab === 'timeline' ? 'text-blue-700 border-blue-600 bg-blue-50/80' : 'text-slate-400 border-transparent hover:text-slate-600 hover:bg-slate-50']"><i class="fa-solid fa-calendar-days text-sm"></i> Lịch Đặt Bàn</button>
      <button @click="ui.tab = 'history'; appStore.loadHistory(false)" :class="['flex-1 py-3 flex justify-center items-center gap-2 transition-all min-h-[48px] border-b-[3px] whitespace-nowrap px-2', ui.tab === 'history' ? 'text-blue-700 border-blue-600 bg-blue-50/80' : 'text-slate-400 border-transparent hover:text-slate-600 hover:bg-slate-50']"><i class="fa-solid fa-list-ul text-sm"></i> Lịch Sử Tạo Phiếu</button>
      <button @click="ui.tab = 'create'" :class="['flex-1 py-3 flex justify-center items-center gap-2 transition-all min-h-[48px] border-b-[3px] whitespace-nowrap px-2', ui.tab === 'create' ? 'text-blue-700 border-blue-600 bg-blue-50/80' : 'text-slate-400 border-transparent hover:text-slate-600 hover:bg-slate-50']"><i class="fa-solid fa-plus text-sm"></i> Tạo Phiếu Đặt</button>
      <button @click="ui.tab = 'preview'" :class="['md:hidden flex-1 py-3 flex justify-center items-center gap-2 transition-all min-h-[48px] border-b-[3px] whitespace-nowrap px-2', ui.tab === 'preview' ? 'text-blue-700 border-blue-600 bg-blue-50/80' : 'text-slate-500 border-transparent hover:text-slate-600 hover:bg-slate-50']"><i class="fa-solid fa-eye text-sm"></i> Xem Phiếu</button>
    </div>
